...
Every agent in agents.py calls an LLM only through the `BaseLLM.generate`
interface, so you can swap backends without touching agent logic.

- MockLLM: deterministic, template-based, dependency-free. This is what
  demo.py uses by default so the whole EA-RAG pipeline is runnable offline,
  without any API key, while still exercising the real orchestration,
  retrieval, graph-traversal, quantitative, and verification logic (the
  parts of the system this paper actually contributes). Only the free-text
  phrasing of claims/explanations is templated rather than generated. It
  does not call any model, local or remote -- it just returns the last
  line of the prompt it was given.

- OllamaLLM (recommended open-source default, easiest setup): talks to a
  local Ollama server running an open-weight model. Default here is
  `gemma3:4b` -- small enough to run CPU-only on a laptop.

- HuggingFaceLLM (recommended open-source default, more control): loads an
  open-weight model directly via `transformers`, e.g. Qwen3-14B, with
  optional 4-bit quantization for smaller GPUs.

- AnthropicLLM: thin wrapper around the `anthropic` Python SDK, for
  plugging in a closed-source model instead.

Why Gemma 3 4B is the *default* here (not asserted as "the best")
------------------------------------------------------------------
"Best open-source LLM" is a moving target that changes most months, and
open-weight-model leaderboards are a heavily SEO'd space -- treat any
single "#1 open model" claim, including ones below, with some skepticism
and check current benchmarks yourself before committing.

What EA-RAG's agents specifically need is: (1) reliable structured-output
/ instruction-following (the planner and reasoner prompts), (2) reasoning
over multi-hop graph facts without inventing new ones, and (3) a size
that runs on hardware a researcher actually has -- for many people that
means a laptop with no dedicated GPU, not a 24GB+ card. `gemma3:4b`
(~3.3GB, Gemma license) is the default for exactly that reason: it runs
at usable speed on CPU alone, needs no quantization flags, and Ollama's
packaged chat template handles system prompts correctly for it even
though Gemma's own template has no native system role (see README.md).
It has no "thinking" mode -- `enable_thinking` should stay False for it.
Depending on your hardware and when you're reading this, reasonable
alternatives (swap the `model` string, nothing else changes) include:
  - Similarly laptop-friendly: "qwen3:4b", "llama3.2:3b"
  - Larger / stronger if you have a GPU or a hosted endpoint: "qwen3:14b"
    or newer Qwen releases, DeepSeek-V3/V4, Llama 4, GLM-4.5+, Kimi K2+ --
    check current specs and licenses before choosing, since these change
    between point releases.
  - If the critic/verifier specifically needs stronger step-by-step
    reasoning, try a reasoning-tuned variant (e.g. "qwen3:14b" with
    enable_thinking=True, or a DeepSeek-R1-distill) for that agent only --
    every agent takes its own `llm` instance, so you can mix models per
    role (see "Mixing models per agent" in README.md).
"""
